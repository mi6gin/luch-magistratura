...
артефакта с production runtime. Пока исследовательская модель обучена на UCI и не
упакована для рабочих данных, её публикация блокируется — она не может незаметно
изменить рекомендации закупок.
